...
public class SetToTagCommandRefactored extends Command {
    ...
    public SetToTagCommandRefactored(
    ...
    public void execute() {
        ...
        if (lockedTagId < 0) {
            ...
            if (resolvedTarget == null) {
                System.out.println("[SetToTag] !!TAG ID=%d HAS NO VALID TARGET IGNORING!!");
        ...
        if (nowSec - lastStatusLogSec >= kStatusLogPeriodSec) {
            ...
            System.out.println("[SetToTag] periodic log");
            System.out.printf ("           tag=%d age=%.2fs src=%s ta=%.2f",
                    lockedTagId,
                    lockAgeSec,
                    lockedSourceTable,
                    lockedSourceArea);
            System.out.printf ("           err=(%.2f, %.2f | %.2f m, %.2f deg)",
                    errorX, 
                    errorY, 
                    errorNorm, 
                    Math.toDegrees(headingErrorRad));
            System.out.printf ("           poseSrc=%s fO=%.2f fOOk=%b inField=%b stepOk=%b",
                    translationFromFused ? "fused" : "odom", 
                    fusedOdomTranslationDelta, 
                    fusedOdomTranslationConsistent, 
                    fusedInField, 
                    fusedStepAccepted);
            System.out.printf ("           cmd=(%.2f%s, %.2f%s, %.4f%s)", 
                    translationX, saturatedX ? "*" : "", 
                    translationY, saturatedY ? "*" : "",
                    omega, 
                    saturatedOmega ? "*" : "");
            System.out.printf ("           meas=(%.2f, %.2f, %.3f) atTrans=%b atHead=%b%n", 
            ...
            if (kEnableVerboseStatusLog) {
                ...
                System.out.printf ("                   target=(%.2f, %.2f, %.2f deg)",
                        targetX,
                        targetY,
                ...
                System.out.printf ("                            contCur %.2f deg, contTgt %.2f deg)%n",
