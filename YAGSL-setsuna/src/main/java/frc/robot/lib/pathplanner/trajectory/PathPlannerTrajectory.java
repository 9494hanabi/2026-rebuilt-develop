...
public class PathPlannerTrajectory {
    ...
    public PathPlannerTrajectory(
        ...
        if (path.isChoreoPath()) {
            ...
            Translation2d startVel =
                    new Translation2d(
                            startingSpeeds.vxMetersPerSecond, startingSpeeds.vyMetersPerSecond);
            ...
            // speed.
            double speed =
                    startVel.getX() * startHeading.getCos()
                            + startVel.getY() * startHeading.getSin();
            if (speed < 0.0) {
                speed = 0.0;
            }
            ...
            startingSpeeds =
                    new ChassisSpeeds(
                            startVel.getX(), startVel.getY(), startingSpeeds.omegaRadiansPerSecond);
            ...
            for (int m = 0; m < config.numModules; m++) {
                states.get(0).moduleStates[m].speedMetersPerSecond =
            ...
            states.get(0).linearVelocity =
                    Math.hypot(
                            fieldStartingSpeeds.vxMetersPerSecond,
                            fieldStartingSpeeds.vyMetersPerSecond);
            ...
            Translation2d endSpeedTrans =
                    new Translation2d(
                            ...
                            states.get(states.size() - 1).heading);
            ...
            for (EventMarker marker : path.getEventMarkers()) {
                if (marker.command() != null) {
                    unaddedEvents.add(
                            new ScheduleCommandEvent(marker.position(), marker.command()));
                }
                if (marker.endPosition() >= 0.0) {
                    // This marker is zoned
                    if (marker.command() != null) {
                        unaddedEvents.add(
                                new CancelCommandEvent(marker.endPosition(), marker.command()));
                    }
                    unaddedEvents.add(
                            new TriggerEvent(marker.position(), marker.triggerName(), true));
                    unaddedEvents.add(
                            new TriggerEvent(marker.endPosition(), marker.triggerName(), false));
                } else {
                    unaddedEvents.add(
                            new OneShotTriggerEvent(marker.position(), marker.triggerName()));
                }
            }
            for (PointTowardsZone zone : path.getPointTowardsZones()) {
                ...
                unaddedEvents.add(
                        new PointTowardsZoneEvent(zone.maxPosition(), zone.name(), false));
            }
            ...
            for (int i = 1; i < states.size(); i++) {
                ...
                if (Math.abs(sumV) < 1e-6 || Math.abs(state.deltaPos) < 1e-6) {
                    ...
                    ChassisSpeeds robotSpeeds =
                            ChassisSpeeds.fromFieldRelativeSpeeds(
                                    state.fieldSpeeds, state.pose.getRotation());

                    double chassisAccelX =
                            (robotSpeeds.vxMetersPerSecond - prevRobotSpeeds.vxMetersPerSecond)
                                    / dt;
                    double chassisAccelY =
                            (robotSpeeds.vyMetersPerSecond - prevRobotSpeeds.vyMetersPerSecond)
                                    / dt;
                    ...
                    double angularAccel =
                            (robotSpeeds.omegaRadiansPerSecond
                                            - prevRobotSpeeds.omegaRadiansPerSecond)
                                    / dt;
                    double angTorque = angularAccel * config.MOI;
                    ChassisSpeeds chassisForces =
                            new ChassisSpeeds(chassisForceX, chassisForceY, angTorque);

                    Translation2d[] wheelForces =
                    ...
                    Translation2d centripetalAccelFieldFrame =
                            new Translation2d(
                                    centripetalAccelMagnitude,
                                    state.heading.rotateBy(Rotation2d.kCCW_90deg));
                    Translation2d centripetalAccelRobotFrame =
                            centripetalAccelFieldFrame.rotateBy(
                    ...
                    ChassisSpeeds chassisForcesCentripetal =
                            new ChassisSpeeds(
                                    centripetalForceRobotFrame.getX(),
                                    centripetalForceRobotFrame.getY(),
                                    0.0);
                    ...
                    Translation2d icrRobotFrame =
                            Math.abs(state.curvature) > 1e-6
                                    ? new Translation2d(
                                            ...
                                            centripetalForceRobotFrame.getAngle())
                                    : Translation2d.kZero;
                    ...
                    for (int m = 0; m < config.numModules; m++) {
                        ...
                        double centripetalRatio =
                                icrDistance > 1e-6 ? icrDistModule / icrDistance : 0.0;
                        ...
                        forceXFF[m] =
                                wheelForces[m].getX()
                                        + centripetalRatio * wheelForcesCentripetal[m].getX();
                        forceYFF[m] =
                                wheelForces[m].getY()
                                        + centripetalRatio * wheelForcesCentripetal[m].getY();
                        ...
                        double torqueCurrent =
                                config.moduleConfig.driveMotor.getCurrent(wheelTorque);
                    ...
                    prevState.feedforwards =
                            new DriveFeedforwards(
                                    accelFF, linearForceFF, torqueCurrentFF, forceXFF, forceYFF);
                }
                ...
                while (!unaddedEvents.isEmpty()
                        && Math.abs(
                                        unaddedEvents.element().getTimestampSeconds()
    ...
    private static void generateStates(
        ...
        for (int i = 0; i < path.numPoints(); i++) {
            ...
            Rotation2d holonomicRot =
                    cosineInterpolate(prevRotationTargetRot, nextRotationTargetRot, t);
            ...
            for (int m = 0; m < config.numModules; m++) {
                if (i != states.size() - 1) {
                    ...
                    states.get(i).moduleStates[m].fieldAngle =
                            states.get(i - 1).moduleStates[m].fieldAngle;
                    states.get(i).moduleStates[m].angle =
                            states.get(i)
                                    .moduleStates[m]
                                    .fieldAngle
        ...
        for (int i = 1; i < states.size() - 1; i++) {
            ...
            for (int m = 0; m < config.numModules; m++) {
                ...
                double availableTorque =
                        config.moduleConfig.driveMotor.getTorque(currentDraw)
                ...
                Translation2d forceVec =
                        new Translation2d(forceAtCarpet, state.moduleStates[m].fieldAngle);
            ...
            double angularAccel =
                    MathUtil.clamp(totalTorque / config.MOI, -maxAngAccel, maxAngAccel);
            ...
            double lonXYAccelLimit =
                    Math.max(
                            Math.abs(
                                    state.constraints.maxXAccelMPSSq()
                                            * effectiveAccelDirection.getCos()),
                            Math.abs(
                                    state.constraints.maxYAccelMPSSq()
                                            * effectiveAccelDirection.getSin()));
            ...
            if (maxDT > 0) {
                ...
                for (int m = 0; m < config.numModules; m++) {
                    ...
                    state.moduleStates[m].speedMetersPerSecond =
                            nextState.moduleStates[m].deltaPos / maxDT;
                }
            }
        ...
        for (int i = states.size() - 2; i > 0; i--) {
            ...
            for (int m = 0; m < config.numModules; m++) {
                ...
                Translation2d forceVec =
                        new Translation2d(
                                forceAtCarpet,
                                state.moduleStates[m].fieldAngle.plus(Rotation2d.k180deg));
            ...
            double maxAccel =
                    Math.min(
                            state.constraints.maxAccelerationMPSSq(),
                            Math.abs(accelLimitsVec.getX()));
            double accel = accelVec.getNorm();
            if (accel > maxAccel) {
                accelVec = accelVec.times(maxAccel / accel);
            }
            ...
            if (maxDT > 0) {
                ...
                for (int m = 0; m < config.numModules; m++) {
                    Rotation2d prevRotDelta =
                            state.moduleStates[m].angle.minus(
                                    states.get(i - 1).moduleStates[m].angle);
                    if (Math.abs(prevRotDelta.getDegrees()) >= 60) {
                        continue;
                    }
    ...
    private static void desaturateWheelSpeeds(
        ...
        if (Math.abs(maxRotationSpeed) > 1e-8) {
            rotationPct =
                    Math.abs(desiredSpeeds.omegaRadiansPerSecond) / Math.abs(maxRotationSpeed);
        }
