...
public class SetToTagCommand extends Command {
    ...
    private static final double kSearchLogPeriodSec = 0.50;
    private static final double kWarningLogPeriodSec = 0.50;
    ...
    public void execute() {
        ...
        double fusedOdomDx = fusedX - odomX;
        double fusedOdomDy = fusedY - odomY;
        double fusedOdomTranslationDelta = Math.hypot(fusedOdomDx, fusedOdomDy);
        double fusedOdomHeadingDiffRad = MathUtil.angleModulus(fusedHeadingRad - odomHeadingRad);
        ...
        if (nowSec - lastStatusLogSec >= kStatusLogPeriodSec) {
            ...
            if (kEnableVerboseStatusLog) {
                System.out.printf(
                        "[SetToTag][detail] poseF=(%.2f, %.2f, %.2f deg) poseO=(%.2f, %.2f, %.2f deg) "
                                + "fused-odom=(%.3f, %.3f, %.2f deg)%n",
                        fusedX,
                        fusedY,
                        ...
                        fusedOdomDx,
                        fusedOdomDy,
                        Math.toDegrees(fusedOdomHeadingDiffRad));
                System.out.printf(
                        "[SetToTag][detail] target=(%.2f, %.2f, %.2f deg) "
                                + "heading=(wrapCur %.2f deg, wrapTgt %.2f deg, contCur %.2f deg, contTgt %.2f deg, err %.2f deg)%n",
                        targetX,
                        targetY,
                        ...
                        Math.toDegrees(headingErrorRad));
                System.out.printf(
                        "[SetToTag][detail] controlErr=(x %.3f m, y %.3f m, heading %.3f rad)%n",
                        errorX,
                        errorY,
                        headingErrorRad);
            }
